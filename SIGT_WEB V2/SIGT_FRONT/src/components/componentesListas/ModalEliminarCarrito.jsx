...
export default function ModalEliminarCarrito({ carrito, onClose, onConfirmar }) {
  ...
  // Extrae el body de la respuesta sin anidar try/catch
  ...
  // Llama onConfirmar con fallback, sin anidar lógica en el flujo principal
  const invocarOnConfirmar = () => {
  ...
  // Maneja el cierre del modal de forma segura
  const cerrarModal = () => {
    try {
      onClose();
    } catch (e) {
  ...
  // Muestra alerta de éxito y ejecuta callbacks
  const manejarExito = () => {
  ...
  // Muestra alerta de error del servidor
  const manejarErrorServidor = (result, response) => {
    const message =
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      manejarErrorServidor(result, response);
    } catch (error) {
