...
const handleSeleccionarTalla = (v) => {
  if (v.stock > 0) {
    setTallaSel(v.talla);
    setCantidad(1);
  }
...
{varsFiltradas.map(v => (
  <button
    key={v.talla}
    onClick={() => handleSeleccionarTalla(v)}
    disabled={v.stock === 0}
    style={getEstiloTalla(v)}
  >
                    {v.talla}</button>
                  ))}
                </div>
              ...
              <button
                ...
                onClick={() => onAgregarAlCarrito(producto, colorSel, tallaSel, cantidad)}
              >
...
function TiendaLine() {
  ...
  const cargarProductos = async () => {
    try {
      ...
      if (res.status === 401) {
        setLoading(false);
        Swal.fire({ icon:"warning", title:"Sesión Expirada", text:"Por favor inicia sesión nuevamente.", confirmButtonColor:"#3085d6" })
      ...
      console.log("Productos:", lista.length, "| Ejemplo:", JSON.stringify(lista[0], null, 2));
      
      setProductosAgrupados(lista);
      console.log("VARIANTES[0]:", JSON.stringify(lista[0]?.variantes, null, 2));
      console.log("COLORES[0]:", JSON.stringify(lista[0]?.colores, null, 2));
  ...
  const agregarAlCarrito = (producto, colorSel, tallaSel, cantidad) => {
    if (!tallaSel) {
      Swal.fire({ icon:"warning", title:"Selecciona una talla", confirmButtonColor:"#f39c12" });
      return;
    }

    const variante = (producto.variantes || [])
      .filter(v => v.color === colorSel)
      .find(v => v.talla === tallaSel);

    if (!variante || variante.stock === 0) {
      Swal.fire({ icon:"error", title:"Sin stock", text:"Esta combinación no tiene stock.", confirmButtonColor:"#d33" });
      return;
    }
    if (cantidad > variante.stock) {
      Swal.fire({ icon:"warning", title:"Stock insuficiente", text:`Solo hay ${variante.stock} disponibles.`, confirmButtonColor:"#f39c12" });
      return;
    }

    const imagenMostrar = variante.imagenes?.[0] || producto.imagen_principal || "/img/no-image.png";
    ...
    Swal.fire({
      ...
      text:`${cantidad} × ${producto.nombre} (${tallaSel}) al carrito.`,
  ...
  const getPortada = (producto) => {
    // Buscar primera imagen real en variantes
