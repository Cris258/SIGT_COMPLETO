    ...
    <>
      ...
      <div className="lc-wrapper">
        ...
        <div className="lc-card">
          ...
          {totalPaginas > 1 && (
            <>
              <p className="lc-page-info">Mostrando {inicio + 1}–{Math.min(inicio + POR_PAGINA, clientesFiltrados.length)} de {clientesFiltrados.length} clientes</p>
              <div className="lc-pagination">
                ...
                {Array.from({ length: totalPaginas }, (_, i) => i + 1)
                  ...
                  .map((item, i) => item === "..." ? <span key={`d${i}`} style={{ color:"#b895d4", padding:"0 4px", lineHeight:"36px" }}>…</span> : <button key={item} className={`lc-pg-btn${paginaSegura===item?" active":""}`} onClick={() => setPaginaActual(item)}>{item}</button>)}
