...
const EmpleadoPage = () => {
  ...
  const ordenarYFiltrarTareas = () => {
    ...
    tareasOrdenadas.sort((a, b) => {
      ...
      const prioridadEstadoA = getPrioridadEstado(estadoA);
      const prioridadEstadoB = getPrioridadEstado(estadoB);
      if (prioridadEstadoA !== prioridadEstadoB) return prioridadEstadoA - prioridadEstadoB;
    ...
    if (searchQuery.trim() === "") {
      ...
      setTareasFiltradas(tareasOrdenadas.filter((tarea) =>
  ...
  const actualizarEstadoTarea = async (idTarea, nuevoEstado) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:3001/api/tarea/${idTarea}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      ...
      if (response.ok) {
        setTareas(prevTareas =>
          prevTareas.map(t => t.idTarea === idTarea ? { ...t, EstadoTarea: nuevoEstado } : t)
        );
      } else {
        console.error(result.Message || "Error al actualizar estado");
      }
    } catch (error) {
