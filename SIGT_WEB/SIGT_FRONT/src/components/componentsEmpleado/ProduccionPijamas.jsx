...
export default function RegistrarProduccion() {
  ...
  const cambiarEstadoTarea = async (idTarea, nuevoEstado) => {
    try {
      ...
      if (response.ok) {
        // Función de ordenamiento (misma que en cargarTareas)
        const ordenarTareas = (tareas) => {
          return tareas.sort((a, b) => {
            const fechaA = a.FechaVencimiento ? new Date(a.FechaVencimiento) : new Date('2099-12-31');
            const fechaB = b.FechaVencimiento ? new Date(b.FechaVencimiento) : new Date('2099-12-31');
        ...
        if (nuevoEstado === "En Progreso") {
          ...
          if (tareaActualizada) {
            const nuevasPendientes = tareasPendientes.filter(t => t.idTarea !== idTarea);
            const nuevasEnProgreso = ordenarTareas([...tareasEnProgreso, { ...tareaActualizada, EstadoTarea: "En Progreso" }]);
            
            setTareasPendientes(nuevasPendientes);
            setTareasEnProgreso(nuevasEnProgreso);
          }
        } else if (nuevoEstado === "Pendiente") {
          // Mover de en progreso a pendientes
          const tareaActualizada = tareasEnProgreso.find(t => t.idTarea === idTarea);
          if (tareaActualizada) {
            const nuevasEnProgreso = tareasEnProgreso.filter(t => t.idTarea !== idTarea);
            const nuevasPendientes = ordenarTareas([...tareasPendientes, { ...tareaActualizada, EstadoTarea: "Pendiente" }]);
